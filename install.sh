...
if ! command -v ipset &> /dev/null || ! command -v wget &> /dev/null; then
    echo -e "${YELLOW}Устанавливаю ipset и wget...${NC}"
    apt-get update -qq 2>/dev/null || true
    apt-get install -y -qq ipset wget >/dev/null 2>&1 || {
        echo -e "${RED}Не удалось установить зависимости. Установи вручную: apt install ipset wget${NC}"
        exit 1
    }
fi
